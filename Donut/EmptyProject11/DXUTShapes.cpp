...
static const D3D11_INPUT_ELEMENT_DESC s_ShapeLayout[] =
    {
...
//--------------------------------------------------------------------------------------
HRESULT CreateShapeMesh( ID3D11Device* pDev11, DXUTMesh** ppMesh, VERTEX* pVertices, UINT NumVertices,
                         WORD* pIndices, UINT NumIndices )
{
    HRESULT hr = S_OK;

    // Create the mesh
    *ppMesh = new(std::nothrow) DXUTMesh();
    if (!*ppMesh)
      return E_OUTOFMEMORY;
    hr = (*ppMesh)->Create(pDev11,
                           sizeof(VERTEX),
                           pVertices,
                           NumVertices,
                           pIndices,
                          ...
                          float fRadius2, float fLength, UINT uSlices, UINT uStacks )
{
    ...
    for( i = 0; i < uSlices; i++ )
        sincosf( 2.0f * XM_PI * i / uSlices, sinI + i, cosI + i );
    ...
    for( j = 0; j <= uStacks; j++ )
    {
        ...
        {
            pVertex->pos = XMFLOAT3( fRadius * sinI[i], fRadius * cosI[i], fZ );
            pVertex->norm = XMFLOAT3( fNormalXY * sinI[i], fNormalXY * cosI[i], fNormalZ );
            pVertex++;
            if( pPointReps != NULL )
                pPointReps[iVertex] = iVertex;
            iVertex++;
...
static void MakePolygon( VERTEX* pVertices, WORD* pwIndices, float fLength, UINT uSides )
{
    ...
    float radius = fLength * 0.5f / sinf( XM_PI / ( float )uSides );
    float angle = ( float )( 2.0f * XM_PI / ( float )uSides );
    ...
    for( UINT j = 0; j < uSides; j++ )
    {
        ...
        pVertex->pos = XMFLOAT3( c * radius, s * radius, 0.0f );
        pVertex->norm = XMFLOAT3( 0.0f, 0.0f, 1.0f );
        pVertex++;
    }
...
static void MakeSphere( VERTEX* pVertices, WORD* pwIndices, float fRadius, UINT uSlices, UINT uStacks )
{
    ...
    for( j = 0; j < uStacks; j++ )
        sincosf( XM_PI * j / uStacks, sinJ + j, cosJ + j );
    ...
    for( j = 1; j < uStacks; j++ )
    {
        for( i = 0; i < uSlices; i++ )
        {
            ...
            pVertex->pos.x = norm.x * fRadius;
            pVertex->pos.y = norm.y * fRadius;
            pVertex->pos.z = norm.z * fRadius;
            pVertex->norm = norm;
...
static void MakeTorus( VERTEX* pVertices, WORD* pwIndices, float fInnerRadius, float fOuterRadius, UINT uSides, UINT uRings )
{
    ...
    for( i = 0; i < uRings; i++ )
    {
        float theta = ( float )i * 2.0f * XM_PI / ( float )uRings;
        float st, ct;
        ...
        for( j = 0; j < uSides; j++ )
        {
            float phi = ( float )j * 2.0f * XM_PI / uSides;
            float sp, cp;
                                ...
                                UINT uRings, DXUTMesh** ppMesh )
{
    ...
    // Set up the defaults
    if( 0.0f == fInnerRadius )
        fInnerRadius = 1.0f;
    if( 0.0f == fOuterRadius )
        fOuterRadius = 2.0f;
    if( 0 == uSides )
        uSides = 8;
    if( 0 == uRings )
...
static XMFLOAT3* teapotPositions = ( XMFLOAT3* )teapotPositionsFloats;
...
static XMFLOAT3* teapotNormals = ( XMFLOAT3* )teapotNormalsfloats;
